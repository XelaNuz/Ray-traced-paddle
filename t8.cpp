...
class Global {
    public:
        ...
        Global() {
            ...
            vecMake(d, d, d,lights[nlights].diffuse);
            vecMake(40000.0,200000.0,0.0,lights[nlights].pos); //NNED TO EDIT
            lights[nlights].shadow = false;
            nlights++;
...
void init(void)
{
    ...
    /*    o = &g.object[g.nobjects];
          o->clear_clips();    
          o->type = TYPE_SPHERE;
          vecMake(40.0, 150.0, 50.0, o->center);
          vecMake(.2,.2,.2, o->color);
          o->radius = 75.0;
          o->surface = SURF_NONE;
          vecMake(0.0, 100.0, 0.0, o->clip[o->nclips].sCenter);
          o->clip[o->nclips].radius = 75;
          vecMake(0.0,1.0,1.0, o->color);
          o->inside = true;
          o->specular = false;
          o->clip[o->nclips].inside = true;
          o->nclips++;
          g.nobjects++;
    ...
    //vecMake(600.0, 280.0, 600.0, g.from);
...
void trace(Ray *ray, Vec rgb, Flt weight, int level)
{
    ...
    if(closehit.t > 2000) { //the 2000 dictates my view distance
    ...
    //-----------------
    //
    Flt fog = closehit.t/2000;
    fog = pow(fog,1.75);
    rgb[0] = rgb[0]*(1.0-fog)+.6*fog;
    rgb[1] = rgb[1]*(1.0-fog)+.6*fog;
