...
class Global {
    public:
        ...
        Global() {
            ...
            vecMake(.8,.8,.8,lights[nlights].diffuse);
            vecMake(0.0,500.0,0.0,lights[nlights].pos);
            nlights++;
           /* vecMake(.8,.8,.8,lights[nlights].diffuse);
            vecMake(500.0,500.0,500.0,lights[nlights].pos);
            nlights++;
            vecMake(.8,.8,.8,lights[nlights].diffuse);
            vecMake(-500.0,500.0,500.0,lights[nlights].pos);
            nlights++;
...
void init(void)
{
    ...
    //--------------------------------------------------------------------
    //Light object
    /*   l = &g.lights[g.nlights];
         vecMake(.8,.8,.8,l->diffuse);
         vecMake(-100.0,1000,-1000,l->pos);
         g.nlights++;*/
    //--------------------------------------------------------------------
    //setup light and camera
    //    vecMake(550.0, 450.0, -500.0, g.lightPos);
...
void getShadow(Hit *z, Vec rLight){
    Light *l;
    ...
    for(int h =0; h < g.nlights; h++) {
        ...
        if (dot < 0) dot =0;
        Flt specHigh = pow(dot, 30)/2;
            if (!inShadow ) {
                rLight[0] += (dot * l->diffuse[0] + specHigh)/4;
                rLight[1] += (dot * l->diffuse[1] + specHigh)/4;
                rLight[2] += (dot * l->diffuse[2] + specHigh)/4;
...
void trace(Ray *ray, Vec rgb, Flt weight, int level)
{
    ...
    if (o->surface == SURF_CHECKER) {
        int x = 1000 + (closehit.p[0]/2) / 20.0;
        int y = 1000 + (closehit.p[2]/2) / 20.0;
        x = x % 2;
        y = y % 2;
        if (x==y) {
    ...
    Vec rLight;
    getShadow(&closehit, rLight);
        rgb[0] += closehit.color[0] * rLight[0];
        rgb[1] += closehit.color[1] * rLight[1];
        rgb[2] += closehit.color[2] * rLight[2];
    return;
}
